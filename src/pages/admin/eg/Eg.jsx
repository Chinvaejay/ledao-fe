...
import {uploadFile, addEg, delEg, getEgList} from "../../../api";
...
class Eg extends Component {
    ...
    render() {
        let {title, url, egList} = this.state;
        return (
            <div>
                <Tabs type="card">
                    <TabPane tab="案例列表" key="1">
                        <Table columns={
                            [
                                { title: 'id',width: 100, dataIndex: 'id', key: 'id', },
                                { title: '案例标题',width: 100, dataIndex: 'title', key: 'title',},
                                    ...
                                    } },
                                {
                                    ...
                                    render: (text, record, index) => {
                                        return <span>
                                            <Button onClick={()=>this.delEg(text, record, index)} type="danger" size="small">删除</Button>
                                        </span>
                                    },
                                },
                            ]
                    ...
                    <TabPane tab="添加案例" key="2">
                        <ul className={'ul'}>
                            <li>
                                <div className="left">案例标题：</div>
                                <div className="right">
                                    <Input onChange={e => this.valueChange(e, 'title')} placeholder="请输入案例标题"
                                           value={title}/>
                                </div>
                            </li>
                            <li>
                            ...
                            <li>
                                <div className="left"/>
                                <div className="right">
                                    <Button onClick={() => this.addEg()} type="primary">添加案例</Button>
                                </div>
                            </li>
                        </ul>
                    </TabPane>
    ...
    delEg = async (text, record, index)=>{
        confirm({
            title: '删除案例?',
            content: `确认删除此案例吗？`,
            onOk: async () =>{
                let data = await delEg({
                    id: text.id
                });
                if(data.code === 200){
                    message.success(data.data);
                    let {egList} = this.state;
                    egList.splice(index, 1);
                    this.setState({
                        egList
                    })
                }else {
