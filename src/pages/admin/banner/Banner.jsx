...
import {uploadFile, addBanner, delBanner, getBannerList} from "../../../api";
...
class Banner extends Component {
    ...
    render() {
        let {title, url, bannerList} = this.state;
        return (
            <div>
                <Tabs type="card">
                    <TabPane tab="轮播图列表" key="1">
                        <Table columns={
                            [
                                ...
                                { title: '轮播图标题',width: 100, dataIndex: 'title', key: 'title',},
                                { title: '轮播图地址',width: 100, dataIndex: 'url', key: 'url', render: text => <a
                                        href={text} target="_blank">
                                        <img src={text} style={{width: '100%', height: '5rem', objectFit: 'cover'}} alt=""/>
                                    </a> },
                                    ...
                                    } },
                                {
                                    ...
                                    render: (text, record, index) => {
                                        return <span>
                                            <Button onClick={()=>this.delBanner(text, record, index)} type="danger" size="small">删除</Button>
                                        </span>
                                    },
                                },
                            ]
                    ...
                    <TabPane tab="添加轮播图" key="2">
                        <ul className={'ul'}>
                            <li>
                                <div className="left">轮播图名称：</div>
                                <div className="right">
                                    <Input onChange={e => this.valueChange(e, 'title')} placeholder="请输入轮播图名称"
                                           value={title}/>
                                </div>
                            </li>
                            <li>
                                <div className="left">上传轮播图：</div>
                                <div className="right">
                                    <div className="img">
                                        {
                            ...
                            <li>
                                <div className="left"/>
                                <div className="right">
                                    <Button onClick={() => this.addBanner()} type="primary">添加轮播图</Button>
                                </div>
                            </li>
                        </ul>
                    </TabPane>
    ...
    delBanner = async (text, record, index)=>{
        confirm({
            title: '删除轮播图?',
            content: `确认删除此轮播图吗？`,
            onOk: async () =>{
                let data = await delBanner({
                    id: text.id
                });
                if(data.code === 200){
    ...
    addBanner = async ()=>{
        ...
        if(!url){
            return message.warning('请上传轮播图')
        }
        let data = await addBanner({
            title, url, adminId
        });
        if(data.code === 200){
            this.getBannerList();
